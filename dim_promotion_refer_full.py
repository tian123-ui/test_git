...
def execute_promotion_refer_etl(partition_date: str):
    ...
    (
        `id` STRING COMMENT '促销参考ID',
        `refer_name` STRING COMMENT '参考名称',
    ...
    # 执行数据加载
    insert_sql = f"""
    INSERT OVERWRITE TABLE dim_promotion_refer_full PARTITION(ds='{partition_date}')
    SELECT 
        `id`,
        `refer_name`,
        `create_time`,
    ...
    print(f"正在加载数据到dim_promotion_refer_full，分区ds={partition_date}")
    spark.sql(insert_sql)

    # 验证数据
    print("数据验证：")
    spark.sql(f"SELECT * FROM dim_promotion_refer_full WHERE ds='{partition_date}' LIMIT 5").show()
